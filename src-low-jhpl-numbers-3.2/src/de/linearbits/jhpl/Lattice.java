...
public class Lattice<T, U> {
    ...
    public static Map<String, Integer> numHasProperty = new HashMap<String, Integer>();
    public static Map<String, Integer> numPutProperty = new HashMap<String, Integer>();
    public static Map<String, Integer> maxAntichain   = new HashMap<String, Integer>();
    public static Map<String, Integer> hitrate        = new HashMap<String, Integer>();
    
    private boolean addHit(PredictiveProperty property) {
        Integer value = hitrate.get(property.getLabel());
        hitrate.put(property.getLabel(), value == null ? 1 : value + 1);
        return true;
    }
    
    private void trackAntichain(PredictiveProperty property, JHPLTrie trie) {
        
        int currentAntichainLength = 0;
        Iterator<int[]> iterator = trie.iterator();
        @SuppressWarnings("unused")
        int[] element = null;
        while ((element = iterator.next()) != null) {
            currentAntichainLength++;
        }
        Integer value = maxAntichain.get(property.getLabel());
        maxAntichain.put(property.getLabel(), value == null ? currentAntichainLength : Math.max(currentAntichainLength, value));
    }
    ...
    public boolean hasProperty(int[] node, int level, PredictiveProperty property) {
        ...
        Integer value = numHasProperty.get(property.getLabel());
        numHasProperty.put(property.getLabel(), value == null ? 1 : value + 1);
        
        switch (property.getDirection()) {
        case UP:
            JHPLTrie trie = this.propertiesUp.get(property);
            return trie == null ? false : (trie.contains(node, level) == true ? addHit(property) : false);
        case DOWN:
    ...
    public void putProperty(int[] node, int level, PredictiveProperty property) {
        ...
        Integer value = numPutProperty.get(property.getLabel());
        numPutProperty.put(property.getLabel(), value == null ? 1 : value + 1);
        ...
        if (hasProperty(node, level, property)) {
            numHasProperty.put(property.getLabel(), numHasProperty.get(property.getLabel()) - 1);
            return;
        }
        numHasProperty.put(property.getLabel(), numHasProperty.get(property.getLabel()) - 1);
